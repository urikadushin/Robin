...
export const MassPropertiesTab: React.FC<MassPropertiesTabProps> = ({ threat }) => {
    ...
    const MassRow = ({ item }: { item: MassMomentAndXCG }) => (
        <div className="grid grid-cols-[1fr_120px_120px_80px] items-center p-4.5 px-6 border-b border-[#ECF2F6] hover:bg-[#EFF6FB] transition-colors group">
            ...
            <div className="flex flex-col gap-1 text-right pr-4">
                <span className="text-[10px] text-[#6b788e] uppercase font-bold">Launch</span>
                <span className="text-[15px] text-[#144a54] font-bold">{item.launch_value?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0.00'}</span>
            </div>
            <div className="flex flex-col gap-1 text-right pr-4">
                <span className="text-[10px] text-[#6b788e] uppercase font-bold">EOB</span>
                <span className="text-[15px] text-[#227d8d] font-bold">{item.eob_value?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0.00'}</span>
            </div>
            <div className="text-[11px] font-bold text-[#6b788e] uppercase text-right">
                {item.unit || '---'}
            </div>
        </div>
    );
    ...
    return (
        <div className="flex flex-col gap-10 animate-in fade-in duration-700 pb-10" style={{ fontFamily: "'Inter', sans-serif" }}>
            <h3 className="text-[12px] font-bold text-[#144a54] uppercase tracking-widest flex items-center gap-2">
                ...
                Mass, Moments & XCG Data Matrix
            </h3>

            <div className="flex flex-col border border-[#C7D8E6] bg-white rounded-[8px] overflow-hidden shadow-sm">
                <div className="grid grid-cols-[1fr_120px_120px_80px] items-center px-6 py-4 bg-[#ECF2F6] border-b border-[#C7D8E6]">
                    <span className="text-[11px] font-bold text-[#144a54] uppercase tracking-widest">Component Analysis</span>
                    <span className="text-[11px] font-bold text-[#144a54] uppercase tracking-widest text-right pr-4">Launch Profile</span>
                    <span className="text-[11px] font-bold text-[#144a54] uppercase tracking-widest text-right pr-4">Burnout (EOB)</span>
                    <span className="text-[11px] font-bold text-[#144a54] uppercase tracking-widest text-right">Units</span>
                </div>
            ...
            <div className="p-5 border-l-4 border-[#227d8d] bg-[#227d8d]/5 rounded-r-[8px] flex items-center gap-4">
                <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center shadow-sm">
                    <span className="text-[#227d8d] font-bold text-[12px]">i</span>
                </div>
                <p className="text-[11px] text-[#227d8d] uppercase font-bold tracking-widest leading-relaxed">
                    EOB = End of Burn State | Values calibrated based on latest flight telemetry and structural mass moment calculations.
                </p>
            </div>
        </div>
    );

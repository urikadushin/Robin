...
export const AeroTab: React.FC<AeroTabProps> = ({ threat }) => {
    ...
    const CoeffItem = ({ label, value, description }: { label: string, value: string | number | undefined, description: string }) => (
        <div className="border border-[#C7D8E6] bg-white p-5 rounded-[8px] flex flex-col gap-3 group hover:border-[#227d8d]/40 transition-all shadow-sm">
            <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-[#227d8d] uppercase tracking-wider">{label}</span>
                <span className="text-[18px] font-bold text-[#144a54]">{typeof value === 'number' ? value.toFixed(4) : value || '0.0000'}</span>
            </div>
            <p className="text-[10px] text-[#6b788e] leading-tight uppercase font-bold">{description}</p>
        </div>
    );

    return (
        <div className="flex flex-col gap-10 animate-in fade-in duration-700" style={{ fontFamily: "'Inter', sans-serif" }}>
            <h3 className="text-[12px] font-bold text-[#227d8d] uppercase tracking-widest flex items-center gap-2">
            ...
            <div className="border border-[#C7D8E6] bg-white p-6 rounded-[8px] shadow-sm">
                <h4 className="text-[11px] font-bold text-[#6b788e] mb-6 uppercase tracking-widest border-b-2 border-[#ECF2F6] pb-2 inline-block">Reference Vectors</h4>
                <div className="grid grid-cols-2 gap-10">
                    <div className="flex flex-col gap-4">
                        <span className="text-[10px] text-[#6b788e] uppercase font-bold tracking-widest">Mach Vector</span>
                        <div className="flex flex-wrap gap-2">
                            {aero.mach_vec?.split(',').map((val: string, idx: number) => (
                                <div key={idx} className="bg-[#ECF2F6] p-2.5 px-4 rounded-[4px] text-[11px] font-bold text-[#144a54] text-center border border-[#C7D8E6] min-w-[50px]">{val}</div>
                            ))}
                        </div>
                    </div>
                    <div className="flex flex-col gap-4">
                        <span className="text-[10px] text-[#6b788e] uppercase font-bold tracking-widest">Alpha Vector (DEG)</span>
                        <div className="flex flex-wrap gap-2">

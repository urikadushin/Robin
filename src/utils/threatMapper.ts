...
export const mapBackendToFrontend = (missile: any, weightAndSize: any[], aerodynamics: any[], performance: any[] = [], engines: any[] = []): ThreatData => {
    ...
    return {
        ...
        speed: performance?.[0]?.velEndOfBurn ? `${Math.round(performance[0].velEndOfBurn)} m/s` : (maxVel > 0 ? `${Math.round(maxVel)} m/s` : 'Unknown'),
        weight: (getVal(['weight', 'launchWeight', 'launch_weight'])) ? `${getVal(['weight', 'launchWeight', 'launch_weight'])} kg` : 'Unknown',
        countries: getVal('origin') || missile.family_type || 'Unknown',
        ...
        payloadWeight: getVal(['payloadWeight', 'whWeight', 'whweight', 'wh_weight', 'payload_weight', 'whWeightExplosive']),
